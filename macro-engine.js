/* ATLAS FX - macro-engine.js
   Derives the top header strip (Bias/Conviction/Risk/USD/Volatility) and Trade Status block
   (Bias/Conviction/Flow/Regime/Validity) from the four macro driver charts - DXY/US10Y/EQUITIES/USDJPY.
   All values computed live from the loaded macro bars - no static values. */
(function(){
var A = window.ATLAS;
...
A.Macro = {
  ...
  update: function(){
    ...
    var conv = convictionScore([mDxy.dir, -mE.dir, mY.dir, mJ.dir].map(Math.sign));
    var riskScore = 0;
    ...
    set("mh-conv", conv.label,  "driver alignment", conv.cls);
    ...
    set("mh-vol",  volLabel,    "sigma "+vol.toFixed(3)+"%", volCls);
    ...
    A.Macro.state = {
      ...
      moves:{dxy:mDxy, y10:mY, eq:mE, jpy:mJ}, flow:flowLabel
    };
  }
};
function macroRun(){
